import { n as __commonJSMin, r as __toESM, t as require_react } from "./react-hMYwPxoE.js";
import { t as require_react_dom } from "./react-dom-vhSoCgKN.js";
		...
		while (cursor < length) {
			...
			switch (segment[0]) {
				...
				case SEGMENT_TYPE_PARAM: {
					...
					const existingNode = node.dynamic?.find((s) => s.caseSensitive === actuallyCaseSensitive && s.prefix === prefix$1 && s.suffix === suffix);
					if (existingNode) nextNode = existingNode;
					else {
				...
				case SEGMENT_TYPE_OPTIONAL_PARAM: {
					...
					const prefix$1 = !prefix_raw ? void 0 : actuallyCaseSensitive ? prefix_raw : prefix_raw.toLowerCase();
					const suffix = !suffix_raw ? void 0 : actuallyCaseSensitive ? suffix_raw : suffix_raw.toLowerCase();
					const existingNode = node.optional?.find((s) => s.caseSensitive === actuallyCaseSensitive && s.prefix === prefix$1 && s.suffix === suffix);
					if (existingNode) nextNode = existingNode;
					else {
...
function useAwaited({ promise: _promise }) {
	if (import_react.use) return import_react.use(_promise);
